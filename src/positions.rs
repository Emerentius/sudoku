...
#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub(crate) struct Field(pub u8);

#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub(crate) struct Line(pub u8); // row or col

#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub(crate) struct Zone(pub u8); // row, col or field

#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub(crate) struct Slice(pub u8);

#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub(crate) struct Band(pub u8); // RowBand if number is in 0..27, else ColBand
...
#[inline(always)]
#[allow(unused)]
pub(crate) fn neighbours_to<T: IntoNeighbours>(cell_set: T) -> T::Output {
