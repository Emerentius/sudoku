...
fn correct_solution_top50k() {
    let sudokus = read_sudokus( include_str!("../sudokus/Lines/top50k.txt") );
    let solved_sudokus = read_sudokus( include_str!("../sudokus/Lines/solved_top50k.txt") );
    for (i, (sudoku, solved_sudoku)) in sudokus.into_iter().zip(solved_sudokus).enumerate() {
        if let Some(solution) = sudoku.clone().solve_unique() {
            assert_eq!( solved_sudoku, solution);
        } else {
...
fn correct_solution_sudoku17() {
    let sudokus = read_sudokus( include_str!("../sudokus/Lines/sudoku17.txt") );
    let solved_sudokus = read_sudokus( include_str!("../sudokus/Lines/solved_sudoku17.txt") );
    for (i, (sudoku, solved_sudoku)) in sudokus.into_iter().zip(solved_sudokus).enumerate() {
        if let Some(solution) = sudoku.clone().solve_unique() {
            assert_eq!( solved_sudoku, solution);
        } else {
